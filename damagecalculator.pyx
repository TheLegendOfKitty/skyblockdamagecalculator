...
def calculatedamage(*args):
    cdef double basedamage = (5 + int(WeaponDamage.get()) + (int(Strength.get()) / 5)) * (1 + int(Strength.get()) / 100)
    cdef double damagemultiplier = 1 + (int(CombatLevel.get()) * 0.04) + (float((int(SharpnessLevel.get()) * 0.05))) + (
                int(FirstStrikeLevel.get()) * 0.25)
    cdef double finaldamagetemp = basedamage * damagemultiplier * (1 + int(CritDamage.get()))
    FinalDamage.set(str(round(finaldamagetemp)) + " damage")
